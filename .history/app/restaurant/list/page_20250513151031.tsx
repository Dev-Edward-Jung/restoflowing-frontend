...
export default function RestaurantPage() {
  ...
  // 1) 로그인 시 localStorage.setItem('jwtToken', receivedToken) 해 두었다고 가정
  const getJwt = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/restaurant/list`, {
      ...
      .then(data => {
        // API가 배열을 직접 내려주면 data, 
        // 혹시 { restaurants: [...] } 형태면 data.restaurants 로 바꿔주세요.
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/restaurant/save`,
      {
        ...
        body: JSON.stringify({
          ...
          restaurantCity: city.trim(),
        }),
      }
    );

    if (res.ok) {
      // 새로고침 대신 서버에서 온 최신 리스트로 갱신
      const updated = await res.json();
      setRestaurants(prev => [...prev, updated]);
      setName('');
      setCity('');
    } else {
                    ...
                    <table className="table">
                      ...
                      <tbody className="table-border-bottom-0">
                        {loading ? (
                          ...
                          <tr>
                            <td colSpan={2}>No restaurants found.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                      ...
                      <div className="modal-content">
                        <div className="modal-header">
                          <h5 className="modal-title">
                            Would you like to add your restaurant?
                          </h5>
                          <button
                            type="button"
                            className="btn-close"
