          ...
          <div key={category}>
            ...
            {items.map(item => (
              <div key={item.id} className={`d-flex gap-2 mb-2 align-items-center ${item.needNow ? 'alert-danger' : ''}`}>
                ...
                <button className={`btn btn-primary ${item.needNow ? 'alert alert-danger' : ''}`} onClick={() => setCurrentItem(item)} data-bs-toggle="modal" data-bs-target="#editModal">Edit</button>
              </div>
            ))}
          </div>
        ))}
