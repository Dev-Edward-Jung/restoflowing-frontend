          ...
          <div key={category}>
            ...
            {items.map(item => (
              <div key={item.id} className={`d-flex gap-2 mb-2 align-items-center`}>
                <input className={`form-control ${item.needNow ? 'bg-danger' : ''}`} disabled value={item.name} />
                <input className={`form-control ${item.needNow ? 'bg-danger' : ''}`} disabled value={item.name} disabled value={item.quantity} type="number" />
