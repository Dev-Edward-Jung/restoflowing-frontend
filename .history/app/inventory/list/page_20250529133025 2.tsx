...
const InventoryPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async (pageToLoad = 0) => {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/inventory/list/paged?restaurantId=${restaurantId}&page=${pageToLoad}`, {
  ...
  }, [observerRef.current, page, isLastPage]);
