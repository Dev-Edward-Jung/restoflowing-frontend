...
const InventoryPage = () => {
  ...
  const [newItem, setNewItem] = useState<Partial<InventoryItem>>({ name: '', quantity: 0, unit: '', categoryId: 0, needThis: false });
  ...
  const addItem = async () => {
    ...
    if (res.ok) {
      ...
      setNewItem({ name: '', quantity: 0, unit: '', categoryId: 0, needThis: false });
    } else {
  ...
  const updateItem = async () => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/inventory/update`, {
      ...
      body: JSON.stringify({ ...currentItem, restaurantId, needThis: currentItem.needThis ?? false }),
    });
            ...
            <div className="modal-content">
              <div className="modal-body">
                ...
                <div className="form-check mt-1">
                  <input className="form-check-input" type="checkbox" checked={newItem.needThis || false} onChange={e => setNewItem(p => ({ ...p, needThis: e.target.checked }))} id="addCheck" />
              ...
              {currentItem ? (
                <>
                  <div className="modal-body">
                    ...
                    <div className="form-check mt-1">
                      <input className="form-check-input" type="checkbox" checked={!!currentItem.needThis} onChange={e => setCurrentItem(p => p ? { ...p, needThis: e.target.checked } : null)} id="editCheck" />
